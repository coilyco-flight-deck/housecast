...
def test_a_declaration_derives_two_slots_sharing_one_pair_id() -> None:
    derived = derive_challenges(load_declaration(DECLARATION), "paired")
...
def test_the_file_level_entity_reaches_every_pair() -> None:
    assert all(
        challenge.entity == "echo"
        for challenge in derive_challenges(load_declaration(DECLARATION), "paired")
    )


def test_a_pair_missing_its_outside_half_is_refused() -> None:
...
def test_coverage_names_a_paired_case_no_declaration_derived() -> None:
    derived = derive_challenges(load_declaration(DECLARATION), "paired")
    dataset = [
